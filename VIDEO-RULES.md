...
### 1. 網站預設語言 = 英文
- 所有評論影片**預設為英文口白 + 英文語音**（英文發音）
- 英文版影片是基礎版本，必須最先製作

### 2. 字幕規則（所有影片都適用）
- **預設軟字幕 = 繁中 + 英文雙語**（同一條 cue 兩行：繁中在上、英文在下）
  - 檔名：`{product}-review.bilingual.vtt`
  - 這是寶博指定的站上預設（英文發音 + 雙語軟字幕）
- **另外仍要三語獨立字幕**：EN / zh-TW / zh-CN，給播放器切換
- 不論影片口白是英文還是中文，獨立字幕都要齊
- 字幕檔不可跨語言影片共用（英文口白的中文翻譯 ≠ 中文口白的中文原文，對位不同）

### 3. 頁面顯示規則
- **有 `bilingual.vtt`**：任何語系頁面預設開雙語軟字幕（繁中 + 英文）
- **沒有雙語檔**：繁中頁用 zh-TW、簡中頁用 zh-CN、英文頁用 EN
- **有中文口白影片**（`zh-TW/`）：繁中／簡中頁可播中文口白，字幕仍優先雙語
- 沒有中文口白 → 一律英文口白影片
...
├── {product}-review.mp4              ← 英文口白（預設）
├── {product}-review.bilingual.vtt    ← 預設軟字幕（繁中上、英文下）
...
| 9 | Claude Code + Codex | ⏳ 待補 | ⏳ | ⏳ | ⏳ | ❌ | — | 16:9 | 無 HEYGEN_API_KEY。口白稿 `content/scripts/claude-code-codex-review-en.txt` |
...
| 日期 | 動作 | 備註 |
|------|------|------|
| 2026-08-28 | 預設改為英文口白 + 繁中／英文雙語軟字幕 | 寶博確認；新片必須有 bilingual.vtt |
