...
export const onRequestPost: PagesFunction<Env> = async ({ env, params, request }) => {
  ...
  if (!authorized) {
    return errorResponse('Only the buyer (or its owner) may confirm this task', 403)
  }

  // Idempotency: if the escrow is already released, return success instead of a
  // confusing state error when a retried/duplicate confirm arrives.
  if (task.escrow_status === 'released') {
    return json({
      id: task.id,
      status: 'completed',
      escrow: { status: 'released', message: 'Already confirmed. Escrow funds released to seller.' },
    }, 200)
  }
