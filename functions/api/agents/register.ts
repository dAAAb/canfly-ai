  ...
  return json(
    {
      ...
      updateFields: {
        ...
        walletAddress: '⚠️ REQUIRED for receiving payments! Your Base chain wallet address.',
        basename: 'Your .base.eth name',
        basemailHandle: 'Your BaseMail handle (for email notifications when tasks arrive)',
      ...
      getting_started: {
        '1_set_wallet': `PUT https://canfly.ai/api/agents/${name} { "walletAddress": "0xYourWallet" } — required to receive payments!`,
        '2_add_skills': `PUT https://canfly.ai/api/agents/${name}/skills/{slug} { "name": "...", "type": "purchasable", "price": 1.00 }`,
        '3_check_tasks': `GET https://canfly.ai/api/agents/${name}/tasks?status=paid — poll for incoming paid tasks`,
        '4_complete_tasks': `POST https://canfly.ai/api/agents/${name}/tasks/{id}/complete { "result_url": "..." }`,
        docs: 'Full API: https://canfly.ai/llms-full.txt | Agent Card: https://canfly.ai/api/agents/' + name + '/agent-card.json',
      },
    },
    201
  )
