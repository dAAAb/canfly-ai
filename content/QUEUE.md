...
最後掃描：2026-09-26（每日選題掃描，06:00 Taipei）
...
| P1 | 新款 Mac mini | 硬體 | 兩邊都同意。2026-08-25 發表 M6 / M5 Pro，**2026-09-22 已出貨**（[MacRumors](https://www.macrumors.com/2026/09/22/apple-released-two-new-macs-today/)），起價約 $899。站上已有 `mac-mini-m4` | **更新舊頁** `mac-mini-m4`，確認要不要新 slug | queued |
| P2 | Mac Studio 512GB（約 10 月） | 硬體／預告 | 寶博點名。一般 M5 Max／M5 Ultra **已 9/22 出貨**；**512GB unified memory 仍 late October**。未上市那檔：先「即將推出」規格文 | blog `mac-studio-512gb` | queued |
| P1 | Grok 4.7（xAI API） | 軟體／models | 官方 [model card](https://docs.x.ai/developers/models/grok-4.7) 已上：`grok-4.7`，500k context，$2/$0.50/$6（prompt ≥200k 則 $4/$1/$12），reasoning `low`／`medium`／`high`（預設）／`xhigh`。OpenClaw [xAI provider](https://docs.openclaw.ai/providers/xai) 預設改 `xai/grok-4.7`：`openclaw models set xai/grok-4.7`（舊 `xai/auto` 用 `openclaw doctor --fix`）。OpenRouter 有 `x-ai/grok-4.7`。Grok 4.7 Fast 只在 Cursor／Grok Build，不要另開 Fast 頁。4.6 同價同窗，寫 4.7 當現役、帶一句 4.6；不要另開 4.6 頁。不要跟 Grok Bot 寫成同一頁 | `/apps/models/grok-4-7` + `/learn/grok-4-7` | shipped 2026-09-25（影片待補） |
...
| P1 | Harden AIF | 軟體／skills | PH 9/9。官方 [AIF docs](https://docs.harden.run/)：`curl -fsSL https://aif.harden.run/install.sh \| sh`，再 `aif configure --agent openclaw --validate`。本機 tool-call 防火牆（bridge／block-and-steer）。文件寫支援 OpenClaw 2026.7.1-2；寫稿時核對對 v2026.9.6 是否仍過。站上沒有相近頁；不要跟 AgentWard 寫成同一頁 | `/apps/skills/harden-aif` + `/learn/harden-aif` | shipped 2026-09-22（影片待補；官方基線仍是 2026.7.1-2，頁上要求對 v2026.9.6 再跑 validate） |
...
| `/learn/ollama` + `/learn/ollama-openclaw` + OpenClaw 卡 | 跟上方 2.0／8.2／9.1／9.6 一起改。Muse Glimmer 寫完後加 `ollama launch openclaw --model muse-glimmer` 連結。Qwen 3.8／Gemma 4 不要另開。GPT-5.6 Sol/Terra/Luna 仍可選；新鮮 OpenAI setup 預設 Astra。GPT-6 Sol／Luna 寫進 OpenClaw／Astra 家族一句：`openclaw models set openai/gpt-6-sol`／`openai/gpt-6-luna`，不要另開頁。Ollama 官方 8/31：Pro／Max／Team 改成含額度的 per-token 計價，寫 Ollama 頁時改價，不要另開。 |
| `/learn/claude-code-codex`（待寫稿） | OpenAI 2026-08-28：Cursor 合約預計 2026-11-12 停供 OpenAI 模型。寫進 Codex 段備註，不要另開 Cursor 頁。Anthropic 2026-09-01：[Fable 5.1／Mythos 5.1](https://www.anthropic.com/claude-fable-and-mythos-5-1) 上線（`claude-fable-5-1`、cache read 降 75%）；OpenRouter 已有 `anthropic/claude-fable-5.1`。寫 Claude Code 段用 Fable 5.1，不要另開模型頁。Mythos 5.1 只走 trusted access；EFS 今秋才 GA。Anthropic 2026-09-22：[Claude Opus 5.5](https://www.anthropic.com/claude-opus-5-5)（`claude-opus-5-5`，$4/$20、cache read $0.20；官方說多數工作接近 Fable 5.1、比 Opus 5 便宜約 40%）。OpenClaw：`openclaw models set anthropic/claude-opus-5-5`（新鮮 Claude setup 預設 Opus 5.5）。寫 Claude Code 段帶一句，不要另開 Opus 頁。OpenAI 2026-09-03：[GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model) 上線（`gpt-6-astra`）；OpenClaw 用 `openclaw models set openai/gpt-6-astra`。Codex 段帶一句並連到 Astra 模型頁，不要把 Astra 折進這篇當主文。OpenAI 2026-09-22：[GPT-6 Sol／Luna](https://developers.openai.com/api/docs/changelog)（`gpt-6-sol` $2/$10、`gpt-6-luna` $0.10/$0.50）；OpenClaw [models](https://docs.openclaw.ai/providers/openai/models)：`openclaw models set openai/gpt-6-sol`／`openai/gpt-6-luna`。Codex 段帶一句並連 Astra 家族，不要另開 Sol／Luna 頁。OpenAI 2026-09-10：[Agents API](https://developers.openai.com/api/docs/guides/agents-api/overview) public beta（託管 Codex harness，`gpt-6-astra`）；是對手不是技能，Codex 段帶一句即可，不要另開頁。Harden AIF 寫完後可在這頁加一句本機 tool-call 防火牆連結。 |
| `/apps/models/gpt-6-astra` + `/learn/gpt-6-astra` | 現有文案寫「OpenRouter 還沒有」。OpenRouter 已上 `openai/gpt-6-astra` 與 `openai/gpt-6-astra-pro`。2026-09-22 同家族 [GPT-6 Sol／Luna](https://developers.openai.com/api/docs/changelog) 已上 OpenRouter（`openai/gpt-6-sol`、`openai/gpt-6-luna`）與 OpenClaw。改這兩頁加家族連結，不要另開 Pro／Sol／Luna 頁。 |
...
| Grok 4.7 獨立頁（舊觀察） | 2026-09 官方 [model card](https://docs.x.ai/developers/models/grok-4.7) 已上，OpenClaw 預設 `xai/grok-4.7`。已改寫既有 Grok 4.6 待寫稿，不要另開第二頁 | 見待寫 Grok 4.7 |
...
| GPT-6 Sol／Luna 獨立頁 | 2026-09-22 官方 changelog：`gpt-6-sol` $2/$10、`gpt-6-luna` $0.10/$0.50。OpenClaw 9.6＋[models](https://docs.openclaw.ai/providers/openai/models) 已支援；OpenRouter 已上。新鮮 setup 仍預設 Astra。跟當年 GPT-5.6 Sol/Terra/Luna 一樣折進 OpenClaw／Astra 家族 | 不要另開；見更新舊頁 Astra／OpenClaw |
| Claude Opus 5.5 獨立頁 | 2026-09-22 官方 [Opus 5.5](https://www.anthropic.com/claude-opus-5-5)：`claude-opus-5-5`，$4/$20。OpenClaw：`openclaw models set anthropic/claude-opus-5-5`。跟 Fable 5.1 一樣折進 Claude Code 稿 | 不要另開 |
| Gemini 3.8 Flash TTS／Flash-Lite TTS | 2026-09-23 官方 TTS，沒有 OpenClaw 技能。跟 Lyria 3.5 同一類 | 不要另開；折進 Gemini 舊頁一句 |
| Perplexity Skills Marketplace／HP ZBook Ultra G3a | Computer 技能市集＋約 10 月 HP 預裝。沒有 `openclaw plugins install` | 寫進 Computer／Portable 待寫稿一句；不要另開 |
| Product Hunt 9/21–9/24（Sai、Arcjet、Hyrax AI、Clueso MCP、WZRD、Anomalo、Plane Agents、MiMo-V2.6、Solid 等） | 對手 computer-use／runtime 安全／MCP 影片／專案管理 agent／託管 agent。沒有可重現的 `openclaw plugins install`／`ollama launch openclaw`。Arcjet 跟已上線 Harden AIF 互搶 tool-call 防火牆，但沒有官方 OpenClaw 接法 | 下週 PH |
| Hindsight（vectorize-io） | GitHub 熱。官方 [OpenClaw 文件](https://hindsight.vectorize.io/sdks/integrations/openclaw)：`openclaw plugins install @vectorize-io/hindsight-openclaw` 再 `npx --package @vectorize-io/hindsight-openclaw hindsight-openclaw-setup`（Cloud／External API／本機 embedded）。跟已入列 Supermemory、觀察中的 OzBrain 互搶長期記憶；先寫 Supermemory | Supermemory 上線後再比；不要第三頁 |
| Product Hunt 9/25（Floot MCP、NOAN、Maximem Synap、Opencontroller、IntellAgents、Harness Manager、GBrain） | SaaS／MCP／agent 治理／對手 harness。Maximem 有舊 plugin `openclaw plugins install @maximem/memory-plugin`（npm 停在 7 月、週載約 19），跟 Supermemory 互搶記憶。NOAN 是 grounding API，沒有 OpenClaw 安裝。GBrain 是對手 harness | 下週 PH |
| Gemini 3.8 Live Avatar | 2026-09-24 官方，只在 Gemini Enterprise，沒有 OpenClaw 技能 | 不要另開；折進 Gemini 舊頁一句 |
| Google AX（Agent Executor） | GitHub 熱。K8s 分散式 agent runtime／對手 harness，沒有 `openclaw plugins install` | 不要另開 |
| Gemini Connected Apps（9/23） | Gemini app 連 Airtable／Linear／Adobe 等，沒有 OpenClaw 安裝步驟 | 不要寫；不必為此改 Gemini 舊頁 |
...
| `google-gemini` | 更新 CLI → Antigravity，不要另開 Gemini 頁。Omni 1.1 Flash、3.8 Flash、3.8 Live、3.8 Flash TTS、3.8 Live Avatar 也寫這頁 |
| `nemotron-3-super` | 不要改寫成 Lightning；加家族連結 |
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋v2026.9.3＋v2026.9.4＋v2026.9.5＋v2026.9.6＋星數（約 390K／390,509）＋custodian，不要重開 2.0／8.2／9.1／9.2／9.3／9.4／9.5／9.6 頁。Astra 另開模型頁。GPT-Live 1 折進 Talk 一句，不要另開語音頁。Gemini 3.8 Live 折進 voice-call 一句。Grok 4.7／GPT-6 Sol／Luna／Opus 5.5 各一句，不要另開 |
...
| `gpt-6-astra` | 2026-09-04 產品＋教學已上。影片待補。補 OpenRouter `openai/gpt-6-astra`／`openai/gpt-6-astra-pro`。GPT-6 Sol／Luna 加家族連結，不要另開 Sol／Luna／Pro 頁。不要跟 Codex／GPT-5.6／Grok Bot 寫成同一頁 |
| `harden-aif` | 2026-09-22 產品＋教學已上。影片待補。官方 pin：`aif configure --agent openclaw --validate`。基線仍寫 2026.7.1-2（2026-08-26），頁上要求對 v2026.9.6 再 validate。不要跟 AgentWard 寫成同一頁 |
...
| 2026-09-25 | 每日選題掃描 | 待寫 +0（既有 Grok 4.6 稿改寫成 4.7）；更新舊頁 +OpenClaw [v2026.9.6](https://docs.openclaw.ai/releases/2026.9.6)、GPT-6 Sol／Luna、Opus 5.5、Mac mini 9/22 出貨、Gemini TTS | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI [Grok 4.7 card](https://docs.x.ai/developers/models/grok-4.7)＋OpenClaw `openclaw models set xai/grok-4.7`、Perplexity 9/21 Skills Marketplace／HP ZBook（折進 Computer／Portable）、OpenAI 9/22 [GPT-6 Sol／Luna](https://developers.openai.com/api/docs/changelog)＋OpenClaw `openai/gpt-6-sol`／`openai/gpt-6-luna`、Anthropic 9/22 [Opus 5.5](https://www.anthropic.com/claude-opus-5-5)、Google 9/23 [3.8 Flash TTS](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-8-text-to-speech/)、PH 9/21–9/24（Sai、Arcjet、Hyrax、Clueso MCP、Solid 等，無 OpenClaw 安裝）、GitHub OpenClaw [v2026.9.6](https://docs.openclaw.ai/releases/2026.9.6)／390,410、ClawHub（無新官方 skill 頁）、Ollama（無新 `ollama launch openclaw`）、Apple mini／Studio 9/22 已出貨（512GB 仍 late October）、NVIDIA Orin Nano 2（H1 2027）、Arduino VENTUNO Q 官方店仍 pre-order／約 4 週 €298.99。檔從 9/21 補到今天。沒寫文章。 |
| 2026-09-26 | 每日選題掃描 | 待寫 +0；更新舊頁 +Gemini Live Avatar 一句、OpenClaw 星數 390,509、VENTUNO Q 交期改 3–4 週 | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI docs（仍 `grok-4.7`，無新品）、Perplexity changelog（Skills Marketplace／effort／Portable 已記）、OpenAI changelog（9/22 之後無新品）、Anthropic newsroom（9/23 起是酶／Accenture／生命科學文，無新品）、Google 9/24 [Live Avatar](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-8-live-with-live-avatar/)（Enterprise）＋9/23 Connected Apps、PH 9/25（Floot MCP、NOAN、Maximem Synap、Opencontroller 等）、GitHub trending（Hindsight／google/ax／Univer／paperclip）、OpenClaw 仍 [v2026.9.6](https://docs.openclaw.ai/releases/2026.9.6)／390,509、ClawHub（無新官方 skill 頁）、Ollama（無新 `ollama launch openclaw`）、Apple mini／Studio（9/22 已出貨；512GB 仍 late October）、NVIDIA Orin Nano 2（H1 2027）、Arduino VENTUNO Q 官方店仍 pre-order／約 3–4 週 €298.99。觀察：Hindsight（官方 plugin，先寫 Supermemory）、PH 9/25、google/ax。沒寫文章。 |
