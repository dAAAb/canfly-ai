...
最後掃描：2026-08-30（每日選題掃描，06:00 Taipei）
...
| P1 | Arduino VENTUNO Q | 硬體 | 寶博點名。新開發板，可接本地／邊緣 Agent。2026-08-25 官方開放預購（Ubuntu 預裝），跟已上市的 UNO Q 不是同一塊板 | `/apps/hardware` + `/learn` | queued |
...
| P1 | Claude Code / Codex 接 OpenClaw | 軟體／skills | 官方 `openclaw mcp serve` 文件直接點名。一頁講兩個 coding agent，不要拆兩篇。不要寫已宣布下線的 AgentKit。OpenAI 2026-11-12 停供 Cursor 的模型，這頁可順便講怎麼改接 OpenClaw／本地模型，不要另開 Cursor 新聞頁 | `/learn/claude-code-codex` | queued |
...
| P1 | Muse Glimmer（Meta / Ollama） | 軟體／models | Meta Superintelligence Labs 第一個開源模型。30B、Apache 2.0、本地 agent。官方一鍵：`ollama launch openclaw --model muse-glimmer`。站上有 Ollama、Nemotron，沒有 Muse 頁。不要跟 Ollama 產品頁寫成同一篇 | `/apps/models/muse-glimmer` + `/learn/muse-glimmer` | queued |
...
| OpenClaw 產品卡 | 文案寫 300K+ stars。GitHub 2026-08-30 約 388K。Custodian skill library 是新能力。 |
| `/learn/nemotron-3-super` | 加 Lightning 家族連結。Super 給重推理，Lightning 給長跑執行層。 |
| `/learn/agent-skills` | ClawHub 產品頁上線後，nextStepCards 要指過去。 |
| `/learn/ollama` + `/learn/ollama-openclaw` | 官方現在推 `ollama launch openclaw --model muse-glimmer`（也有 `qwen3.8`）。現有頁模型名單還沒寫。Muse 另開模型頁，這裡只加推薦與連結。 |
...
| 題目 | 缺什麼 | 下次再看 |
|---|---|---|
| Claude Cowork | 2026-08-26 desktop 加了內建瀏覽器，仍沒看到 OpenClaw 接法 | 有官方接法再入列 |
| Google Antigravity 獨立頁 | Gemini 舊頁更新就夠講 CLI 搬家。先看它跟 OpenClaw 會不會互踩 | I/O 後續文件 |
| OpenAI AgentKit | 2026-11-30 下線 | 不要寫 |
| OpenAI 切斷 Cursor 模型 | 2026-11-12 停供，是新聞不是產品。寫進已入列的 Claude Code / Codex 頁即可 | 不要另開 |
| Anthropic Model Hardware Standard (MHS) | 2026-08-27 研究預覽、申請制。Raspberry Pi / LeRobot 是 partner，沒有公開 OpenClaw 安裝步驟 | 開源或有 Pi 驅動文件再看 |
| Cloudways Managed OpenClaw | 官方一鍵部屬有，但跟現有 `zeabur` 託管頁重疊 | 先更新 zeabur 對照，不要另開 |
| Qwen 3.8 獨立頁 | Ollama 有 `ollama launch openclaw --model qwen3.8`，但跟 Muse Glimmer / Lightning 會互搶「本地 27–30B agent 模型」。寫進 Ollama 舊頁 | 不要另開 |
...
| `heygen` / `elevenlabs` / `ollama` / `omlx` / `zeabur` / `openrouter` | 教學已在，只在有大改版時回訪。Ollama 頁補 Muse Glimmer / qwen3.8 推薦，不要把 Muse 整篇塞進來 |
...
| 2026-08-30 | 每日選題掃描 | 待寫 +1（Muse Glimmer）；更新舊頁 +Ollama 加 Glimmer/qwen3.8 | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI/Grok 4.6（已入列）、Perplexity OpenClaw 文件／Portable（已入列）、OpenAI Cursor 合約（不寫產品）、Anthropic MHS 預覽＋Cowork 瀏覽器（觀察）、Google Antigravity／Omni（更新舊頁）、PH 8/29 無新 OpenClaw 安裝題、GitHub OpenClaw ~388K、ClawHub、Apple mini/Studio（已入列）、NVIDIA Jetson H1 2027、Arduino VENTUNO Q 預購已開（已入列）。沒寫文章。 |
