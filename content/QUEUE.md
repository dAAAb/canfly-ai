...
最後掃描：2026-09-07（每日選題掃描，06:00 Taipei）
...
| `/learn/ollama` + `/learn/ollama-openclaw` + OpenClaw 卡 | 跟上方 2.0／8.2／9.1／9.2 一起改。Muse Glimmer 寫完後加 `ollama launch openclaw --model muse-glimmer` 連結。Qwen 3.8／Gemma 4 不要另開。GPT-5.6 Sol/Terra/Luna 仍是預設；Astra 另開模型頁後加連結（OpenRouter `openai/gpt-6-astra` 已上）。Ollama 官方 8/31：Pro／Max／Team 改成含額度的 per-token 計價，寫 Ollama 頁時改價，不要另開。Muse Spark 1.3 折進 OpenClaw 9.2 一句，不要另開。 |
| `/learn/claude-code-codex`（待寫稿） | OpenAI 2026-08-28：Cursor 合約預計 2026-11-12 停供 OpenAI 模型。寫進 Codex 段備註，不要另開 Cursor 頁。Anthropic 2026-09-01：[Fable 5.1／Mythos 5.1](https://www.anthropic.com/claude-fable-and-mythos-5-1) 上線（`claude-fable-5-1`、cache read 降 75%）；OpenRouter 已有 `anthropic/claude-fable-5.1`。寫 Claude Code 段用 Fable 5.1，不要另開模型頁。Mythos 5.1 只走 trusted access；EFS 今秋才 GA。OpenAI 2026-09-03：[GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model) 上線（`gpt-6-astra`）；OpenClaw 用 `openclaw models set openai/gpt-6-astra`。OpenRouter 已有 `openai/gpt-6-astra`。Codex 段帶一句並連到 Astra 模型頁，不要把 Astra 折進這篇當主文。 |
| `/apps/models/gpt-6-astra` + `/learn/gpt-6-astra` | 產品頁還寫 OpenRouter 沒有。已上 [`openai/gpt-6-astra`](https://openrouter.ai/openai/gpt-6-astra)。改那句即可，不要重開。 |
...
| Muse Spark 1.1／1.2／1.3 | Meta 雲端 API，權重不開。1.3 已進 OpenClaw [v2026.9.2](https://docs.openclaw.ai/releases/2026.9.2)（`meta/muse-spark-1.3`）。本機路線是 Glimmer | 折進 OpenClaw 9.2／OpenRouter；不要跟 Glimmer 寫成同一頁 |
...
| Product Hunt 9/5（dif.sh、Reflexio、Ponytail、Hyperprobe、Experiential Labs、GitWarren） | 給 coding agent 用的 feature flag／SaaS debug／行為學習。沒有官方 OpenClaw 安裝步驟 | 下週 PH |
| Product Hunt 9/6（AI Toolbox 3.0、Tadata、Notify.domains、Kit by Speakeasy、H3 Max、DocsAlot） | Chrome 聊天匣／Slack coworker／影片。Kit 是 ACP 對手 harness，不是 OpenClaw 技能。PH 上的 Gemini Agentic Video 是 9/1 API 功能，折進 Gemini 舊頁 | 下週 PH；Kit 有官方 OpenClaw 接法再比 |
| NVIDIA PAIR（Personal AI Router） | 官方 [PAIR](https://www.nvidia.com/en-eu/ai-on-rtx/personal-ai-router/) beta：本機 Ollama／LM Studio proxy，把家裡多機推論分流。NVIDIA IFA 文提到 OpenClaw Windows App／RTX，但沒有 `openclaw plugins install` 或獨立 skill 頁。沿用既有 Ollama 端點即可 | 折進 Ollama／硬體頁帶一句；有官方 OpenClaw 文件再決定要不要獨立頁 |
...
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋星數＋custodian，不要重開 2.0／8.2／9.1／9.2 頁。Astra 另開模型頁 |
| `gpt-6-astra` | 2026-09-04 產品＋教學已上。影片待補。產品頁「OpenRouter 還沒有」要改成已有 `openai/gpt-6-astra`。不要跟 Codex／GPT-5.6／Grok Bot 寫成同一頁 |
...
| 2026-09-07 | 每日選題掃描 | 待寫 +0；更新舊頁 +OpenClaw v2026.9.2、Astra OpenRouter、Gemini Agentic Video | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI docs／model card（仍 `grok-4.6`，無 4.7 ID／定價）、Perplexity（Computer／Hybrid／PII 已記，無新 OpenClaw 指令）、OpenAI（Astra 已上；OpenRouter [`openai/gpt-6-astra`](https://openrouter.ai/openai/gpt-6-astra) 已開）、Anthropic（Fable／EFS 已記，newsroom 無新品）、Google（3.8 Flash 已折進舊頁；Agentic Video 9/1 API 折進 Gemini 舊頁）、PH 9/5（dif.sh、Reflexio、Hyperprobe 等）＋9/6（AI Toolbox、Tadata、Kit by Speakeasy、Gemini Agentic Video PH 卡）、GitHub OpenClaw [v2026.9.2](https://github.com/openclaw/openclaw/releases/tag/v2026.9.2) 2026-09-05 20:00 UTC／~388K、ClawHub（無新官方 skill 頁）、Ollama blog（最新仍 8/31 計價）、Apple mini／Studio（9/22）、NVIDIA Orin Nano 2（H1 2027）＋PAIR beta（觀察）、Arduino VENTUNO Q／UNO Q（已入列）。觀察：PH 9/5–9/6、Kit、NVIDIA PAIR、Muse Spark 1.3。沒寫文章。 |
