...
最後掃描：2026-09-16（每日選題掃描，06:00 Taipei）
...
| P1 | Arduino VENTUNO Q | 硬體 | 寶博點名。新開發板，可接本地／邊緣 Agent。2026-09-11 官方 [AMR 應用文](https://blog.arduino.cc/2026/09/11/building-smarter-amrs-with-the-arduino-ventuno-q-board/) 與 2026-09-15 [MYWAI VILMA 合作文](https://blog.arduino.cc/2026/09/15/mywai-vilma-is-designed-to-bring-human-like-learning-to-robots-via-one-shot-demonstration/) 都不是出貨更新；[官方店](https://store.arduino.cc/products/ventuno-q) 仍 pre-order／約 4 週／€298.99 | `/apps/hardware` + `/learn` | queued |
...
| Agent 37 Cloud／AgentSky／Murmell | 託管 OpenClaw／多 harness 雲。跟 Zeabur／Pinata 互搶，沒有獨立 skill 安裝路徑。jurniti（PH 9/15）同類 | 折進 Zeabur；不要另開 |
| Gemma 4 獨立頁 | 2026-04 就有，`ollama launch openclaw --model gemma4:26b`。跟 Qwen 3.8 一樣留在 Ollama | 不要另開 |
| Grok 4.7 | Musk 9/2 說約 10 天（約 9/12）；後來說還要再幾天。2026-09-16 仍無 card：xAI [release notes](https://docs.x.ai/developers/release-notes) 9 月只記 Imagine quality 11/2 下線；[model list](https://docs.x.ai/developers/models) 最新仍 `grok-4.6`；`/developers/models/grok-4.7` 404，沒有 ID、定價、benchmark | 官方 model card 上線再決定是更新 Grok 4.6 稿還是另開 |
...
| Gemini 3.8 Live／Live Extended Thinking 獨立頁 | 2026-09-15 官方語音模型。跟 ElevenLabs／GPT-Live 1／官方 `@openclaw/voice-call` 重疊 | 不要另開；折進 Gemini 舊頁＋OpenClaw voice-call 一句 |
| OzBrain（PH 9/14） | 官方 [OpenClaw 頁](https://ozbrain.com/for/openclaw)：`openclaw mcp add ozbrain --url https://ozbrain.com/api/mcp --transport streamable-http`，再 `openclaw mcp login ozbrain`。託管共用記憶，不是 plugin | 觀察；不要另開記憶頁 |
| Product Hunt 9/14（Nimble、Slashy、Oats、Elva、Aside、Juggler、Naoma、LLMagnet、oh-my-hermes 等） | 沒有可重現的 OpenClaw 安裝步驟。Juggler／Aside／oh-my-hermes 是對手 harness／瀏覽器。Elva 是通用 MCP 目錄。OzBrain 見上方 | 下週 PH |
| Product Hunt 9/15（tiun、Kilo Code mobile、Sierra、is.team、LibreChat、Buddy MCP、OpenAI Agents API 再發、Axari、Voiskey 等） | SaaS／對手／通用 MCP。Agents API 已觀察。jurniti 見下方 | 下週 PH |
| jurniti（PH 9/15） | 託管 OpenClaw Firecracker microVM，$25/mo。跟 Zeabur／Agent 37／AgentSky／Murmell 互搶，沒有獨立 skill 安裝路徑 | 折進 Zeabur；不要另開 |
...
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋v2026.9.3＋v2026.9.4＋星數（約 390K）＋custodian，不要重開 2.0／8.2／9.1／9.2／9.3／9.4 頁。Astra 另開模型頁。GPT-Live 1 折進 Talk 一句；Gemini 3.8 Live 折進 voice-call 一句，不要另開語音頁 |
...
| 2026-09-16 | 每日選題掃描 | 待寫 +0；更新舊頁 +Gemini 3.8 Live | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI docs／model list（仍 `grok-4.6`，`/developers/models/grok-4.7` 404、無 card）、Perplexity blog（Hybrid／Computer／Sonar 9/27 已記，無新品）、OpenAI changelog（9/10 之後無新品）、Anthropic newsroom（仍 Fable 5.1／9/10 威脅情報，無新品）、Google [3.8 Live／Live Extended Thinking](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-8-live-gemini-3-8-live-extended-thinking/)（`gemini-3.8-live`、`gemini-3.8-live-extended-thinking`，折進 Gemini 舊頁＋OpenClaw voice-call）、PH 9/14–9/15（Nimble、OzBrain、jurniti、Sierra、LibreChat 等）、GitHub／docs OpenClaw 仍 [v2026.9.4](https://docs.openclaw.ai/releases/2026.9.4)／389,786、ClawHub（@openclaw 96 plugins，無新官方 skill 頁）、Ollama（無新 `ollama launch openclaw`）、Apple mini／Studio（9/22；Golden Gate 已 9/14 上市）、NVIDIA Orin Nano 2（H1 2027）、Arduino VENTUNO Q 官方店仍 pre-order／約 4 週 €298.99（9/15 VILMA 合作文不是出貨更新）。觀察：OzBrain official MCP、jurniti 託管。沒寫文章。 |
