...
最後掃描：2026-09-18（每日選題掃描，06:00 Taipei）
...
| P0 | Perplexity Portable Computer | 軟體／硬體交界 | 寶博點名。可攜版，長尾詞新。2026-09-14 官方 [Portable Computer for Windows](https://www.perplexity.ai/hub/blog/portable-computer-for-windows-is-here)：GeForce RTX／RTX PRO、≥24GB VRAM、Pro／Max；本機 MCP。寫進這篇，**不要另開 Windows 頁** | blog 先寫，規格穩了再產品頁 | queued |
...
| 題目 | 缺什麼 | 下次再看 |
|---|---|---|
| Claude Cowork | 文件／試算表 agent，沒看到 OpenClaw 接法。2026-09-16 Reuters 寫 Claude One／Docs／Slides（chat＋Cowork 合成一介面）；Anthropic newsroom 沒有對應產品文 | 有官方 OpenClaw 接法再入列；不要另開 One／Docs／Slides 頁 |
...
| Agent 37 Cloud／AgentSky／Murmell／jurniti | 託管 OpenClaw／多 harness 雲。jurniti（PH 9/15）是 Firecracker microVM 上的 managed OpenClaw。跟 Zeabur／Pinata 互搶，沒有獨立 skill 安裝路徑 | 折進 Zeabur；不要另開 |
| Gemma 4 獨立頁 | 2026-04 就有，`ollama launch openclaw --model gemma4:26b`。跟 Qwen 3.8 一樣留在 Ollama | 不要另開 |
| Grok 4.7 | Musk 9/2 說約 10 天（約 9/12）；後來說還要再幾天。2026-09-18 仍無 card：xAI [release notes](https://docs.x.ai/developers/release-notes) 9 月只記 Imagine quality 11/2 下線；[model list](https://docs.x.ai/developers/models) 最新仍 `grok-4.6`；`/developers/models/grok-4.7` 回 404，沒有 ID、定價、benchmark | 官方 model card 上線再決定是更新 Grok 4.6 稿還是另開 |
...
| Product Hunt 9/14（Naoma、Nimble Web Search Agents、Hello Inbox、Elva、Slashy、Oats、Aside、LLMagnet、OzBrain） | SaaS demo／網搜／MCP 目錄／郵件／對手瀏覽器。Nimble 只給通用 `agent-onboarding.md`，跟 Firecrawl／brave-search 互搶。Elva 是通用 MCP 託管。Aside 是對手 agent 瀏覽器 | 下週 PH |
| OzBrain（PH 9/14） | 官方 [OpenClaw 頁](https://ozbrain.com/for/openclaw)：`openclaw mcp add ozbrain --url https://ozbrain.com/api/mcp --transport streamable-http` 再 `openclaw mcp login ozbrain`。託管共用知識，不是 plugin，跟 OpenClaw 本機 memory 重疊 | 不要另開；有獨立 skill／plugin 再比 |
| Product Hunt 9/15（Kilo Code 行動版、siift、Idlen、Axari、OpenAI Agents API、jurniti、Viktor、Airtop 再發） | 對手 coding agent／SaaS／已觀察的 Agents API。jurniti 折進上方託管列 | 下週 PH |
| Product Hunt 9/16（Weave Router 2.0、Appwrite 2.0、Gemini 3.8 Live PH、Toki、Twigg、ZeroClick、Fide Island） | 沒有官方 OpenClaw skill。Weave／Appwrite 是路由／BaaS。Gemini Live 已折進 Gemini 舊頁。Twigg 是通用 context API；ZeroClick 是 agent 結帳 | 下週 PH |
| Product Hunt 9/17（CREEM 2.0、Bitrise Remote Dev Environments、Modaal、Text Agent Store、NovaSynth、MCPJam、Die With Me） | 付款／雲端 Mac CI／對手 app builder／iMessage agent 市集／語音評測／MCP 測試。沒有 `openclaw plugins install` 或 `ollama launch openclaw` | 下週 PH |
| Anthropic 9/17 指標文 | [Measurements for understanding the pace of AI development](https://www.anthropic.com/news) 是政策／透明度文，不是產品 | 不要寫 |
...
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋v2026.9.3＋v2026.9.4＋星數（約 390K）＋custodian，不要重開 2.0／8.2／9.1／9.2／9.3／9.4 頁。Astra 另開模型頁。GPT-Live 1 折進 Talk 一句；Gemini 3.8 Live 折進 voice-call 一句，不要另開語音／Live 頁 |
...
| 2026-09-18 | 每日選題掃描 | 待寫 +0；更新舊頁 +Gemini 3.8 Live、Portable Windows 規格 | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI docs／model list（仍 `grok-4.6`，`grok-4.7` 404、無 card）、Perplexity [Portable Windows](https://www.perplexity.ai/hub/blog/portable-computer-for-windows-is-here)（折進既有 P0）、OpenAI changelog（9/10 之後無新品）、Anthropic newsroom（9/17 指標文不是產品；One／Docs／Slides 只有 Reuters）、Google [3.8 Live](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-8-live-gemini-3-8-live-extended-thinking/)（折進 Gemini／OpenClaw voice-call）、PH 9/14–9/17（OzBrain 官方 MCP、jurniti 託管、Weave／Appwrite／CREEM／Bitrise／MCPJam 等無獨立 skill）、GitHub／docs OpenClaw 仍 [v2026.9.4](https://docs.openclaw.ai/releases/2026.9.4)／389,987、ClawHub（@openclaw 仍 7 skills／96 plugins，無新官方 skill 頁）、Ollama（無新 `ollama launch openclaw`）、Apple mini／Studio（9/22；Golden Gate 已上市）、NVIDIA Orin Nano 2（H1 2027）、Arduino VENTUNO Q 官方店仍 pre-order／約 4 週 €298.99（9/15 VILMA 不是出貨更新）。沒寫文章。 |
