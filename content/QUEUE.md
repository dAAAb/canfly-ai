...
最後掃描：2026-09-24（每日選題掃描，06:00 Taipei／cron 22:00 UTC 9/23）
...
| P0 | Perplexity Computer | 軟體／產品 | 寶博點名。新形態「AI 電腦」，跟現有 Perplexity 搜尋頁要分開。2026-09-01 官方 [Hybrid Compute](https://www.perplexity.ai/hub/blog/pii-trace-detecting-personal-data-before-it-leaves-the-device)：Mac 上雲端＋本機分流，Gemma 4 E4B／Qwen3.6 35B、需 Apple silicon＋24GB。2026-09-17 官方 [effort mode](https://www.perplexity.ai/hub/blog/computer-adds-effort-mode-for-model-selection)：web 上 Light／Standard／High／Ultra 滑桿選模型與推理量（Android／iOS 即將）。2026-09-21 官方 [changelog](https://www.perplexity.ai/changelog/effort-mode-gpt-6-astra-and-skills-marketplace)：Skills Marketplace、Computer 可走 GPT-6 Astra、HP 預裝（ZBook Ultra G3a，約 10 月）。寫 Computer 時帶 Hybrid＋effort＋Marketplace／Astra／HP，不要另開 Marketplace／HP 頁 | `/apps` + blog 對照「Perplexity vs Perplexity Computer」 | queued |
...
| P1 | 新款 Mac mini | 硬體 | 兩邊都同意。2026-08-25 發表 M6 / M5 Pro，**2026-09-22 已出貨**，起價約 $899。站上已有 `mac-mini-m4` | **更新舊頁** `mac-mini-m4`，確認要不要新 slug | queued |
| P2 | Mac Studio 512GB（約 10 月） | 硬體／預告 | 寶博點名。未上市：先「即將推出」規格文 | blog `mac-studio-512gb` | queued |
| P1 | Grok 4.7（xAI API） | 軟體／models | 2026-09-21 官方 [Grok 4.7](https://x.ai/news/grok-4-7)＋[release notes](https://docs.x.ai/developers/release-notes)＋[model card](https://media.x.ai/v1/website/4p7card-5eccc980.pdf)。ID `grok-4.7`，500k，$2/$0.50/$6（<200k；≥200k 為 $4/$1/$12），effort low／medium／high／xhigh。OpenRouter 已有 `x-ai/grok-4.7`。OpenClaw 官方 [xAI](https://docs.openclaw.ai/providers/xai) 目錄仍列 `grok-4.6`，OAuth 預設 `xai/auto` 跟遠端 catalog；寫稿時核對 `openclaw models set xai/grok-4.7` 或 `xai/auto`。**不要寫過期的 4.6。** Fast 只在 Cursor／Grok Build、不在公開 API，不要另開 Fast 頁。不要跟 Grok Bot 寫成同一頁 | `/apps/models/grok-4-7` + `/learn/grok-4-7` | queued |
...
| P1 | Firecrawl（含 Developer Index） | 軟體／skills | 官方有 OpenClaw 接法：`firecrawl init --agent openclaw`，也可 `npx -y firecrawl-cli@latest setup developer-index`。ClawHub 另有官方 plugin：`openclaw plugins install clawhub:@openclaw/firecrawl-plugin`。70M+ repo README／issue／PR／OpenAPI，給 coding agent 用，不是一般網搜。2026-09-22 官方 [Alexandria](https://www.firecrawl.dev/blog/introducing-alexandria-series-b)（知識庫＋licensed providers＋indexes，走既有 Firecrawl API／CLI／MCP）。寫進這頁一句，不要另開 Alexandria 頁。跟現有 `brave-search`、`perplexity` 不是同一頁 | `/apps/skills/firecrawl` + `/learn/firecrawl` | queued |
...
| `/learn/ollama` + `/learn/ollama-openclaw` + OpenClaw 卡 | 跟上方 2.0／8.2／9.1 一起改。Muse Glimmer 寫完後加 `ollama launch openclaw --model muse-glimmer` 連結。Qwen 3.8／Gemma 4 不要另開。GPT-6 Astra 仍是新鮮 OpenAI 預設；Sol／Luna 一句 `openai/gpt-6-sol`／`openai/gpt-6-luna`（$2/$10、$0.10/$0.50），不要另開。Ollama 官方 8/31：Pro／Max／Team 改成含額度的 per-token 計價，寫 Ollama 頁時改價，不要另開。 |
| `/learn/claude-code-codex`（待寫稿） | OpenAI 2026-08-28：Cursor 合約預計 2026-11-12 停供 OpenAI 模型。寫進 Codex 段備註，不要另開 Cursor 頁。Anthropic 2026-09-01：[Fable 5.1／Mythos 5.1](https://www.anthropic.com/claude-fable-and-mythos-5-1) 上線（`claude-fable-5-1`、cache read 降 75%）；OpenRouter 已有 `anthropic/claude-fable-5.1`。寫 Claude Code 段用 Fable 5.1，不要另開模型頁。Mythos 5.1 只走 trusted access；EFS 今秋才 GA。Anthropic 2026-09-22：[Claude Opus 5.5](https://www.anthropic.com/claude-opus-5-5)（`claude-opus-5-5`，$4/$20，cache read $0.20；Sonnet／Haiku 5.5 數週內）。Claude Code 段帶一句，不要另開 Opus 5.5 頁。OpenAI 2026-09-03：[GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model) 上線（`gpt-6-astra`）；OpenClaw 用 `openclaw models set openai/gpt-6-astra`。Codex 段帶一句並連到 Astra 模型頁，不要把 Astra 折進這篇當主文。OpenAI 2026-09-22：[GPT-6 Sol／Luna](https://developers.openai.com/api/docs/changelog)（`gpt-6-sol` $2/$10、`gpt-6-luna` $0.10/$0.50）；OpenClaw 已有 `openclaw models set openai/gpt-6-sol`／`openai/gpt-6-luna`。Codex 段帶一句並連 Astra 頁，不要另開 Sol／Luna 頁。OpenAI 2026-09-10：[Agents API](https://developers.openai.com/api/docs/guides/agents-api/overview) public beta（託管 Codex harness，`gpt-6-astra`）；是對手不是技能，Codex 段帶一句即可，不要另開頁。Harden AIF 寫完後可在這頁加一句本機 tool-call 防火牆連結。 |
| `/apps/models/gpt-6-astra` + `/learn/gpt-6-astra` | 現有文案寫「OpenRouter 還沒有」。OpenRouter 已上 `openai/gpt-6-astra` 與 `openai/gpt-6-astra-pro`。改這兩頁，不要另開 Pro 頁。2026-09-22 官方 [GPT-6 Sol／Luna](https://developers.openai.com/api/docs/changelog)：`gpt-6-sol` $2/$10、`gpt-6-luna` $0.10/$0.50；OpenClaw [models](https://docs.openclaw.ai/providers/openai/models) 已寫 `openclaw models set openai/gpt-6-sol`／`openai/gpt-6-luna`。Astra 頁加家族連結一句，不要另開 Sol／Luna 頁。 |
...
| Grok 4.7 | 2026-09-21 已 GA：官方 card＋`grok-4.7`＋OpenRouter `x-ai/grok-4.7`。已改入列，取代 4.6 待寫 | 見待寫 Grok 4.7 |
...
| Product Hunt 9/21（Jev、Sai、Arcjet） | Jev 是 TypeSafe decision model（`console.typesafe.ai`），OpenClaw 9/22 blog 已折進 OpenClaw 更新。Sai 是電腦使用艦隊，對手不是技能。Arcjet 是 app 內 runtime 安全 SDK，沒有 `openclaw plugins install` | 下週 PH |
| Product Hunt 9/22（Plane Agents、Fez、Clueso MCP、WZRD） | SaaS workspace agent／Mac 多 agent／影片 MCP／文件 harness。Fez 用 Jev 做決策，是對手桌面 app。沒有官方 OpenClaw skill | 下週 PH |
| Product Hunt 9/23（Solid Agents、Naise、AgentScore、Pactto、ToneBird、Alexandria、GBrain、Jev State） | SaaS／行銷 agent／會議。Claude Opus 5.5 折進 Claude Code 稿。Alexandria 折進 Firecrawl 待寫。GBrain 見下方獨立列。Jev State 跟 Jev 同一條 | 下週 PH |
| GBrain（PH 9/23、garrytan/gbrain） | 官方 [INSTALL_FOR_AGENTS.md](https://github.com/garrytan/gbrain/blob/master/INSTALL_FOR_AGENTS.md)：`gbrain skillpack scaffold --all`，也可從 checkout 裝 OpenClaw plugin。是「個人腦＋50+ skills」，跟已入列 Supermemory 互搶長期記憶；先寫 Supermemory | Supermemory 上線後再比；不要第三頁 |
| Googlebook（9/21） | 官方 [$899 筆電](https://blog.google/products-and-platforms/devices/googlebook/googlebook-built-in-intelligence/)，10/4 出貨。內建 Gemini／Antigravity／Spark，是對手裝置不是技能，也不是現有 mini／迷你 PC 後繼 SKU | 不要寫；出貨後若有本機 OpenClaw 路徑再看 |
| Gemini 3.8 Flash TTS／Flash-Lite TTS（9/23） | 官方 [TTS](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-8-text-to-speech/)。語音生成，跟 ElevenLabs 重疊，沒有 OpenClaw 接法 | 不要另開；不必為此改 Gemini 舊頁 |
| GitHub trending 9/22–23（google/ax、univer、substrate、treg、CLI-Anything、superpowers） | 對手 harness／office runtime／tool router／skills 框架，沒有官方 OpenClaw skill。agent-native 已觀察 | 不要另開 |
...
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋v2026.9.3＋v2026.9.4＋v2026.9.5＋星數（約 390K／390,344）＋custodian，不要重開 2.0／8.2／9.1／9.2／9.3／9.4／9.5 頁。Astra 另開模型頁。Sol／Luna 折進 Astra／OpenClaw 一句。GPT-Live 1 折進 Talk 一句，不要另開語音頁。Gemini 3.8 Live 折進 voice-call 一句。decision models／Jev 折進 9.5 更新一句 |
| `gpt-6-astra` | 2026-09-04 產品＋教學已上。影片待補。補 OpenRouter `openai/gpt-6-astra`／`openai/gpt-6-astra-pro`。Sol／Luna 加家族一句，不要另開。不要跟 Codex／GPT-5.6／Grok Bot 寫成同一頁，不要另開 Pro 頁 |
...
| 2026-09-24 | 每日選題掃描 | 待寫 retarget 1（Grok 4.6→4.7）；更新舊頁 +GPT-6 Sol／Luna、Claude Opus 5.5、Computer Skills Marketplace、Firecrawl Alexandria、Mac mini 9/22 出貨、OpenClaw 星數 390,344 | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI [Grok 4.7](https://x.ai/news/grok-4-7) 9/21 GA（`grok-4.7`、$2/$6、OpenRouter `x-ai/grok-4.7`；OpenClaw 目錄仍 4.6、`xai/auto`）、Perplexity 9/21 [changelog](https://www.perplexity.ai/changelog/effort-mode-gpt-6-astra-and-skills-marketplace)（Marketplace／Astra-in-Computer／HP，折進 Computer）、OpenAI 9/22 [Sol／Luna](https://developers.openai.com/api/docs/changelog)（`gpt-6-sol` $2/$10、`gpt-6-luna` $0.10/$0.50；OpenClaw 已有 set 指令，折進 Astra／Codex）、Anthropic 9/22 [Opus 5.5](https://www.anthropic.com/claude-opus-5-5)（`claude-opus-5-5` $4/$20，折進 Claude Code）、Google 9/21 Googlebook＋9/23 [3.8 TTS](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-8-text-to-speech/)（觀察）、PH 9/21–9/23、GitHub trending（google/ax／univer／substrate／treg／CLI-Anything）、OpenClaw 仍 [v2026.9.5](https://docs.openclaw.ai/releases/2026.9.5)／390,344＋9/22 decision models blog、ClawHub（無新官方 skill 頁）、Ollama（無新 `ollama launch openclaw`）、Apple mini 9/22 已出貨、NVIDIA Orin Nano 2（H1 2027）、Arduino VENTUNO Q 官方店仍 pre-order／約 4 週 €298.99。檔從 9/21 補到今天。沒寫文章。 |
