...
最後掃描：2026-09-23（每日選題掃描，06:00 Taipei）
...
| P0 | Perplexity Computer | 軟體／產品 | 寶博點名。新形態「AI 電腦」，跟現有 Perplexity 搜尋頁要分開。2026-09-01 官方 [Hybrid Compute](https://www.perplexity.ai/hub/blog/pii-trace-detecting-personal-data-before-it-leaves-the-device)：Mac 上雲端＋本機分流，Gemma 4 E4B／Qwen3.6 35B、需 Apple silicon＋24GB。2026-09-17 官方 [effort mode](https://www.perplexity.ai/hub/blog/computer-adds-effort-mode-for-model-selection)：web 上 Light／Standard／High／Ultra 滑桿選模型與推理量（Android／iOS 即將）。2026-09-21 官方 [Skills Marketplace](https://www.perplexity.ai/changelog/effort-mode-gpt-6-astra-and-skills-marketplace)：Computer 可瀏覽／安裝 skill，Enterprise 可治理；changelog 同一則還帶 GPT-6 Astra 進 Computer。寫 Computer 時帶 Hybrid＋effort＋Skills／Astra／HP 一句，不要另開 Hybrid／effort／Skills／Marketplace 頁 | `/apps` + blog 對照「Perplexity vs Perplexity Computer」 | queued |
...
| P1 | Grok 4.7（xAI API） | 軟體／models | 2026-09-21 官方上線，改寫未寫的 4.6 稿。xAI [release notes](https://docs.x.ai/developers/release-notes)：`grok-4.7`，500k，$2/$0.50/$6（<200k）／$4/$1/$12（≥200k）；effort low／medium／high（預設）／xhigh。OpenRouter `x-ai/grok-4.7`。OpenClaw xAI 文件仍列 `xai/grok-4.6`；寫稿時核對 picker 有沒有 `xai/grok-4.7`，沒有就用 `xai/auto`。不要另開 4.6 頁。不要另開 Fast（只在 Cursor／Grok Build）。不要跟 Grok Bot 寫成同一頁 | `/apps/models/grok-4-7` + `/learn/grok-4-7` | queued |
...
| `/learn/ollama` + `/learn/ollama-openclaw` + OpenClaw 卡 | 跟上方 2.0／8.2／9.1 一起改。Muse Glimmer 寫完後加 `ollama launch openclaw --model muse-glimmer` 連結。Qwen 3.8／Gemma 4 不要另開。GPT-5.6 Sol/Terra/Luna 仍是文件預設；9/22 GPT-6 Sol／Luna 上線後，寫稿時核對 picker，不要另開 Sol／Luna 頁。Astra 另開模型頁後加連結。Ollama 官方 8/31：Pro／Max／Team 改成含額度的 per-token 計價，寫 Ollama 頁時改價，不要另開。 |
| `/learn/claude-code-codex`（待寫稿） | OpenAI 2026-08-28：Cursor 合約預計 2026-11-12 停供 OpenAI 模型。寫進 Codex 段備註，不要另開 Cursor 頁。Anthropic 2026-09-01：[Fable 5.1／Mythos 5.1](https://www.anthropic.com/claude-fable-and-mythos-5-1) 上線（`claude-fable-5-1`、cache read 降 75%）；OpenRouter 已有 `anthropic/claude-fable-5.1`。寫 Claude Code 段用 Fable 5.1，不要另開模型頁。Mythos 5.1 只走 trusted access；EFS 今秋才 GA。Anthropic 2026-09-22：[Opus 5.5](https://www.anthropic.com/claude-opus-5-5)（`claude-opus-5-5`，$4/$20，號稱接近 Fable 5.1、比 Opus 5 便宜約 40%）；OpenRouter `anthropic/claude-opus-5.5`。OpenClaw Anthropic 文件仍寫 `anthropic/claude-opus-5`；寫稿時核對 picker。折進 Claude Code 段一句，不要另開 Opus 5.5 頁。OpenAI 2026-09-03：[GPT-6 Astra](https://developers.openai.com/api/docs/guides/latest-model) 上線（`gpt-6-astra`）；OpenClaw 用 `openclaw models set openai/gpt-6-astra`。Codex 段帶一句並連到 Astra 模型頁，不要把 Astra 折進這篇當主文。OpenAI 2026-09-10：[Agents API](https://developers.openai.com/api/docs/guides/agents-api/overview) public beta（託管 Codex harness，`gpt-6-astra`）；是對手不是技能，Codex 段帶一句即可，不要另開頁。OpenAI 2026-09-22：GPT-6 Sol／Luna 是 Astra 家族工作馬，Codex 段帶一句即可，不要另開。Harden AIF 寫完後可在這頁加一句本機 tool-call 防火牆連結。 |
| `/apps/models/gpt-6-astra` + `/learn/gpt-6-astra` | 現有文案寫「OpenRouter 還沒有」。OpenRouter 已上 `openai/gpt-6-astra` 與 `openai/gpt-6-astra-pro`。改這兩頁，不要另開 Pro 頁。2026-09-22 OpenAI 上線 GPT-6 Sol／Luna（`gpt-6-sol` $2/$10、`gpt-6-luna` $0.10/$0.50；OpenRouter `openai/gpt-6-sol`、`openai/gpt-6-luna`）。Astra 頁加家族一句，不要另開 Sol／Luna 頁。 |
...
| Grok 4.7 獨立觀察（舊） | 2026-09-21 已 GA：`grok-4.7`＋OpenRouter `x-ai/grok-4.7`，已改寫待寫列 | 見待寫 Grok 4.7 |
...
| Product Hunt 9/21（Jev、Superset Mobile、Sai、Arcjet、Hyrax、SecAIQ） | 沒有可重現的 `openclaw plugins install`。Sai 是對手 computer-use 艦隊；Superset Mobile 是 Codex／對手 coding app；Arcjet 是 Claude Code／Cursor 運行時安全；Jev／Hyrax 是決策 API／程式碼審查 SaaS | 下週 PH |
| Product Hunt 9/22（Grok 4.7、Googlebook、Fez、Plane Agents、Clueprint、WeWeb MCP、Valori、MiMo-V2.6 等） | Grok 4.7 已改寫待寫。其餘 SaaS／對手 harness／MCP／記憶層。Clueprint 是 Mac 監控，沒有 OpenClaw 安裝步驟。ResumeContext／Contextberg 見下方獨立列 | 下週 PH |
| ResumeContext（PH 9/22） | GitHub `openclaw skills install resume-context`，還要 Redis＋`cargo install resume`＋Anthropic key。跟已入列 Supermemory／觀察中 OzBrain／ai-memory 互搶長期記憶 | Supermemory 上線後再比；不要第四頁 |
| Contextberg agent-history（PH 9/22） | 官方 `openclaw mcp set agent-history -- npx -y @contextberg/agent-history --mcp`；companion 是 `npx skills add`。讀本機 Claude Code／Codex／Cursor／OpenClaw 歷史，不是長期記憶 plugin | Supermemory 上線後再比；不要另開 |
| ai-memory（GitHub，akitaonrails） | 官方 `ai-memory install-hooks --agent openclaw --apply` 再 `openclaw plugins install --link`。coding-CLI 交接記憶，跟 Supermemory 互搶 | Supermemory 上線後再比；不要第三頁 |
| Googlebook（9/21） | 官方 [預購文](https://blog.google/products-and-platforms/devices/googlebook/pre-order-googlebook/)：$899 起，美 10/4、其他 10/5。Gemini 筆電，沒有 OpenClaw 安裝步驟，是對手不是技能 | 不要另開；不必為此改 Gemini 舊頁 |
| GPT-6 Sol／Luna 獨立頁 | 2026-09-22 OpenAI changelog：`gpt-6-sol`／`gpt-6-luna`。OpenRouter 已有。OpenClaw 文件仍寫 GPT-5.6。是 Astra 家族工作馬，不是新品頁 | 折進 OpenClaw／Astra／Codex 稿；OpenClaw catalog 有 `openai/gpt-6-sol` 再決定要不要家族連結 |
| Claude Opus 5.5 獨立頁 | 2026-09-22 官方 [Opus 5.5](https://www.anthropic.com/claude-opus-5-5)：`claude-opus-5-5`，$4/$20。OpenRouter `anthropic/claude-opus-5.5`。OpenClaw 仍寫 `anthropic/claude-opus-5`。跟 Fable 5.1 一樣折進 Claude Code 稿 | 不要另開 |
| GitHub trending 9/22–23（google/ax、treg、univer、agent-substrate） | 對手 distributed harness／工具路由／Office harness。沒有官方 OpenClaw skill | 不要另開 |
...
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋v2026.9.3＋v2026.9.4＋v2026.9.5＋星數（約 390K／390,262）＋custodian，不要重開 2.0／8.2／9.1／9.2／9.3／9.4／9.5 頁。Astra 另開模型頁。GPT-6 Sol／Luna、GPT-Live 1 折進 Talk／模型一句，不要另開語音／Sol／Luna 頁。Gemini 3.8 Live 折進 voice-call 一句 |
| `gpt-6-astra` | 2026-09-04 產品＋教學已上。影片待補。補 OpenRouter `openai/gpt-6-astra`／`openai/gpt-6-astra-pro`。GPT-6 Sol／Luna 加家族一句，不要另開。不要跟 Codex／GPT-5.6／Grok Bot 寫成同一頁，不要另開 Pro 頁 |
...
| `heygen` / `elevenlabs` / `ollama` / `omlx` / `zeabur` / `openrouter` | 教學已在，只在有大改版時回訪。Qwen 3.8、Gemma 4、Hy4、Muse Spark、GPT-6 Sol／Luna、Opus 5.5 折進 Ollama／OpenRouter／Astra／Claude Code，不要另開 |
...
| 2026-09-23 | 每日選題掃描 | 待寫 +0（改寫 Grok 4.6→4.7）；更新舊頁 +Opus 5.5、GPT-6 Sol／Luna、Computer Skills Marketplace、Mac mini 9/22 出貨、星數 390,262 | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI [Grok 4.7](https://docs.x.ai/developers/release-notes) 9/21 GA（`grok-4.7`；OpenClaw 仍 `xai/grok-4.6`）、Perplexity 9/21 [Skills Marketplace](https://www.perplexity.ai/changelog/effort-mode-gpt-6-astra-and-skills-marketplace)、OpenAI 9/22 changelog（`gpt-6-sol`／`gpt-6-luna`）、Anthropic 9/22 [Opus 5.5](https://www.anthropic.com/claude-opus-5-5)、Google Googlebook（觀察）、PH 9/21–9/22、GitHub trending（google/ax、treg、univer、substrate）、OpenClaw 仍 [v2026.9.5](https://docs.openclaw.ai/releases/2026.9.5)／390,262、ClawHub（無新官方 skill 頁）、Apple mini 9/22 出貨、NVIDIA Orin Nano 2（H1 2027）、Arduino VENTUNO Q 官方店仍 pre-order／約 4 週 €298.99。補 9/22 掃描未寫進 QUEUE 的部分。沒寫文章。 |
