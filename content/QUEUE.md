...
最後掃描：2026-09-08（每日選題掃描，06:00 Taipei）
...
| `/learn/ollama` + `/learn/ollama-openclaw` + OpenClaw 卡 | 跟上方 2.0／8.2／9.1／9.2 一起改。Muse Glimmer 寫完後加 `ollama launch openclaw --model muse-glimmer` 連結。Qwen 3.8／Gemma 4 不要另開。GPT-5.6 Sol/Terra/Luna 仍是預設；Astra 另開模型頁後加連結。Ollama 官方 8/31：Pro／Max／Team 改成含額度的 per-token 計價，寫 Ollama 頁時改價，不要另開。NVIDIA PAIR（2026-09-03 技術文）是 Ollama／LM Studio 本機網路代理，寫 Ollama 頁帶一句即可。 |
...
| Muse Spark 1.1／1.2／1.3 | Meta 雲端 API，權重不開。1.3 已在 OpenRouter `meta/muse-spark-1.3`，OpenClaw v2026.9.2 內建。本機路線是 Glimmer | 折進 OpenRouter／OpenClaw 9.2 稿；不要跟 Glimmer 寫成同一頁 |
...
| Product Hunt 9/5（dif.sh、Reflexio、Hyperprobe、Ponytail） | 功能旗標／行為學習／prod probe／GitHub 重用。對 Claude Code／Codex／Cursor，沒有官方 OpenClaw 安裝步驟 | 下週 PH |
| Product Hunt 9/6（AI Toolbox 3.0、Tadata、Kit by Speakeasy、Notify.domains、H3 Max、hi.new、MCPHub、SyncAgents） | Chrome 聊天管理／Slack coworker／ACP 對手 harness／影片模型。hi.new 是多人 Grok Bot，寫進已入列 Grok Bot 即可。Kit 沒有 `openclaw plugins install` | 下週 PH；Kit 有官方 OpenClaw 接法再比 |
| Product Hunt 9/7（Nina by Antalpha） | 非託管加密／美股交易助手＋通用 MCP，沒有 OpenClaw skill | 下週 PH |
| NVIDIA PAIR | 官方 [技術文](https://developer.nvidia.com/blog/nvidia-pair-virtual-inference-router-expands-available-compute-on-your-local-network/)：本機網路虛擬推論路由器，代理 Ollama／LM Studio，agent harness 不用改。沒有 `openclaw plugins install`／獨立 skill 頁 | 折進 Ollama／OpenClaw 9.2 稿帶一句；不要另開 |
| Gemini Agentic Video（9/1 API） | 官方 API `processing: "agentic"`，沒有 OpenClaw 接法 | 折進 Gemini 舊頁一句；不要另開 |
| Perplexity 9/4 Fast Embeddings／Ivy-Tulip-ROSE | 搜尋基礎設施文，沒有產品頁或 OpenClaw 指令 | 不要另開 |
...
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋星數＋custodian，不要重開 2.0／8.2／9.1／9.2 頁。Astra 另開模型頁；Spark 1.3／PAIR 只帶一句 |
| `gpt-6-astra` | 2026-09-04 產品＋教學已上。影片待補。OpenRouter 已有 [`openai/gpt-6-astra`](https://openrouter.ai/openai/gpt-6-astra) 與 `openai/gpt-6-astra-pro`；產品卡還寫「OpenRouter 還沒有」，下次改這頁時改掉。不要跟 Codex／GPT-5.6／Grok Bot 寫成同一頁 |
...
| 2026-09-08 | 每日選題掃描 | 待寫 +0；更新舊頁 +OpenClaw v2026.9.2、Astra OpenRouter、Gemini Agentic Video | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI [release notes](https://docs.x.ai/developers/release-notes)／[models](https://docs.x.ai/developers/models)（仍 `grok-4.6`，無 4.7 ID；Imagine `grok-imagine-image-quality` 11/2 退役不另開）、Perplexity（Computer／Hybrid 已入列；9/4 embeddings 基礎設施文無 OpenClaw）、OpenAI changelog 9/3 後無新品（Astra 已上；OpenRouter 現有 `openai/gpt-6-astra`）、Anthropic news（仍 Fable 5.1／EFS）、Google [Agentic Video](https://blog.google/innovation-and-ai/models-and-research/gemini-models/introducing-agentic-video-in-gemini/)＋Antigravity 點名 OpenClaw OAuth、PH 9/5–9/7（dif.sh、Reflexio、Hyperprobe、AI Toolbox、Tadata、Kit、hi.new、Nina）、GitHub OpenClaw [v2026.9.2](https://docs.openclaw.ai/releases/2026.9.2) 2026-09-05 20:00 UTC／~388K、ClawHub（無新官方 skill 頁）、Ollama（無新 `ollama launch openclaw` 模型）、Apple mini／Studio（9/22）、NVIDIA PAIR（Ollama 代理）＋Orin Nano 2（H1 2027）、Arduino VENTUNO Q（經銷寫 9/8 開始出貨，官網仍 pre-order）／UNO Q（已入列）。觀察：PH 9/5–9/7、PAIR、Kit、Muse Spark 1.3。沒寫文章。 |
