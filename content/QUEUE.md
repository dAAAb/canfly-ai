...
最後掃描：2026-09-22（每日選題掃描，06:00 Taipei）
...
| P0 | Perplexity Computer | 軟體／產品 | 寶博點名。新形態「AI 電腦」，跟現有 Perplexity 搜尋頁要分開。2026-09-01 官方 [Hybrid Compute](https://www.perplexity.ai/hub/blog/pii-trace-detecting-personal-data-before-it-leaves-the-device)：Mac 上雲端＋本機分流，Gemma 4 E4B／Qwen3.6 35B、需 Apple silicon＋24GB。2026-09-17 官方 [effort mode](https://www.perplexity.ai/hub/blog/computer-adds-effort-mode-for-model-selection)：web 上 Light／Standard／High／Ultra 滑桿選模型與推理量（Android／iOS 即將）。2026-09-21 官方 changelog [Skills Marketplace](https://www.perplexity.ai/changelog/effort-mode-gpt-6-astra-and-skills-marketplace)：Computer 技能市集＋GPT-6 Astra 進 Computer＋HP 預裝（ZBook Ultra G3a，約 10 月）。寫 Computer 時帶 Hybrid＋effort＋Skills／HP 一句，不要另開 Hybrid／effort／Skills／HP 頁 | `/apps` + blog 對照「Perplexity vs Perplexity Computer」 | queued |
...
| P1 | 新款 Mac mini | 硬體 | 兩邊都同意。2026-08-25 發表 M6 / M5 Pro，起價約 $899。2026-09-22 出貨日到。站上已有 `mac-mini-m4` | **更新舊頁** `mac-mini-m4`，確認要不要新 slug | queued |
| P2 | Mac Studio 512GB（約 10 月） | 硬體／預告 | 寶博點名。未上市：先「即將推出」規格文 | blog `mac-studio-512gb` | queued |
| P1 | Grok 4.7（xAI API） | 軟體／models | 2026-09-21 官方上線，接替已入列未寫的 4.6。官方 [model page](https://docs.x.ai/developers/models/grok-4.7)：`grok-4.7`，500k context，text+image→text，$2/$0.50/$6（<200k）／$4/$1/$12（≥200k）。[Release notes](https://docs.x.ai/developers/release-notes) 寫明長跑 agent／coding。OpenRouter 已有 `x-ai/grok-4.7`。OpenClaw 官方 [xAI 文件](https://docs.openclaw.ai/providers/xai) 目錄仍寫 4.6／`openclaw models set xai/grok-4.6`；寫稿時核對 picker 是否已有 `xai/grok-4.7`，否則 `xai/auto`。Fast 只走 Cursor／Grok Build，不要另開 Fast 頁。不要另開 4.6 頁。不要跟 Grok Bot 寫成同一頁 | `/apps/models/grok-4-7` + `/learn/grok-4-7` | queued |
...
| Grok 4.7 獨立頁（舊觀察） | 2026-09-21 已 GA：官方 `grok-4.7`＋[model page](https://docs.x.ai/developers/models/grok-4.7)＋OpenRouter `x-ai/grok-4.7`。4.6 待寫稿改寫成 4.7 | 見待寫 Grok 4.7 |
...
| GitHub trending 9/20–22（ECC、agent-native、CUA） | 對手 harness／computer-use 框架，沒有官方 OpenClaw skill | 不要另開 |
| Product Hunt 9/21（Hyrax AI、Superset Mobile、Plume、Arcjet、SecAIQ Watch、Sai、Gradio Workflow 等） | SaaS／對手 coding app／履歷。Superset 只接 Claude Code／Codex。Arcjet 官方是 `npx plugins add arcjet/arcjet-plugin`（Claude Code／Cursor），不是 `openclaw plugins install`，跟 Harden AIF 互搶 runtime 安全 | 下週 PH；Arcjet 有官方 OpenClaw plugin 再比 |
| Googlebook（9/21） | 官方 [built-in Gemini](https://blog.google/products-and-platforms/devices/googlebook/googlebook-built-in-intelligence/)：$899 起、10/4 出貨。Magic Pointer／Rambler／Antigravity／Spark。消費筆電＋Gemini 對手，沒有 OpenClaw 安裝步驟 | 不要另開；不必為此改 Gemini 舊頁 |
| ai-memory（GitHub 熱） | 官方 [OpenClaw MCP＋hooks](https://github.com/akitaonrails/ai-memory/blob/main/docs/mcp-install.md)：`ai-memory install-hooks --agent openclaw --apply`（生成本機 plugin 再 `openclaw plugins install --link`）。跨 coding CLI 長期記憶＋handoff。跟剛入列 Supermemory 互搶記憶頁 | Supermemory 上線後再比；不要第三頁 |
| Perplexity Skills Marketplace（9/21） | Computer 內建技能市集＋GPT-6 Astra 進 Computer＋HP 預裝，不是 OpenClaw skill | 寫 Computer 待寫稿帶一句；不要另開 |
...
| `openclaw` | 更新成 2.0（v2026.8.1）＋穩定補丁 v2026.8.2＋v2026.9.1＋v2026.9.2＋v2026.9.3＋v2026.9.4＋v2026.9.5＋星數（約 390K／390,216）＋custodian，不要重開 2.0／8.2／9.1／9.2／9.3／9.4／9.5 頁。Astra 另開模型頁。GPT-Live 1 折進 Talk 一句，不要另開語音頁。Gemini 3.8 Live 折進 voice-call 一句 |
...
| 2026-09-22 | 每日選題掃描 | 待寫 改寫 Grok 4.6→4.7；更新舊頁 +Mac mini 9/22 出貨、OpenClaw 星數 390,216 | 對過 `SOP-NEW-APP.md`、`src/data/products.ts`。來源：xAI 官方 [grok-4.7](https://docs.x.ai/developers/models/grok-4.7)＋[release notes](https://docs.x.ai/developers/release-notes)＋OpenRouter `x-ai/grok-4.7`、Perplexity changelog 9/21（Skills Marketplace／Astra／HP，折進 Computer）、OpenAI changelog（9/15 之後只有 API key governance）、Anthropic newsroom（仍 9/17 LSVP／實驗室，無新品）、Google [Googlebook](https://blog.google/products-and-platforms/devices/googlebook/googlebook-built-in-intelligence/)（觀察）＋3.8 Live 已折進舊頁、PH 9/21、GitHub trending（ai-memory／agent-native／CUA）、OpenClaw 仍 [v2026.9.5](https://docs.openclaw.ai/releases/2026.9.5)／390,216、ClawHub（無新官方 skill 頁）、Ollama（最新仍 8/31 計價）、Apple mini 9/22 出貨、NVIDIA Orin Nano 2（H1 2027）、Arduino VENTUNO Q 官方店仍 pre-order／約 4 週 €298.99。沒寫文章。 |
